...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    async function fetchResearchData(idea) {
        ...
        mobile = user.mobile;
        idea = idea.idea;
        let request = await fetch(`${API_BASE_URL}/api/report/report1`, {
            method: "POST",
            headers: {
            ...
            <div class="content-grid-two-col">
                <div class="card"><div class="data-section"><h3>Infrastructure Plan</h3>
                    <div class="key-metrics-grid">
                        ...
                        <div class="metric-item"><strong>Security</strong><span>${escapeHTML(data.infra_plan.security.join(', '))}</span></div>
                        <div class="metric-item"><strong>Observability</strong><span>${escapeHTML(data.infra_plan.observability.join(', '))}</span></div>
                    </div>
                </div></div>
                <div class="card"><div class="data-section"><h3>Data Model Notes</h3>
                    <div class="key-metrics-grid">
                        <div class="metric-item"><strong>Sources</strong><span>${escapeHTML(data.data_model_notes.sources.join(', '))}</span></div>
                        <div class="metric-item"><strong>Schema Highlights</strong><span>${escapeHTML(data.data_model_notes.schema_highlights.join(', '))}</span></div>
    ...
    function renderTimeBudgetPage(data) {
        ...
        container.innerHTML = `
            <div class="card total-budget-card">
                ...
                <div class="price-range">$${escapeHTML(data.budget_estimate.total_usd.low.toLocaleString())} - $${escapeHTML(data.budget_estimate.total_usd.high.toLocaleString())}</div>
                <div class="table-wrapper"><table class="data-table">
                    <thead><tr><th>Phase</th><th>Low Estimate</th><th>High Estimate</th></tr></thead>
                    <tbody>${data.budget_estimate.by_phase_usd.map(p => `<tr><td class="budget-td">${escapeHTML(p.phase)}</td><td class="budget-td">$${escapeHTML(p.low.toLocaleString())}</td><td class="budget-td">$${escapeHTML(p.high.toLocaleString())}</td></tr>`).join('')}</tbody>
                </table></div>
            </div>
    ...
    async function getCustomerReviews() {
        let realcusrev = await fetch(`${API_BASE_URL}/api/form/getu`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json"
            },
    ...
    navLinks.forEach(link => {
        link.addEventListener('click', (e) => {
            ...
            if (headerContent) {
                headerTextContainer.innerHTML = `<h1>${escapeHTML(headerContent.title)}</h1><p>${escapeHTML(headerContent.subtitle)}</p>`;
            }
